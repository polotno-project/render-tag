...
describe('Layout logic (mocked measureText)', () => {
  ...
  describe('Vertical align', () => {
    ...
    // A shifted span's background/border band belongs to that span, so it has
    // to travel with the glyphs — a band left on the unshifted baseline detaches
    // from its own text. Two colors keep the boxes from merging into one run.
    // Same font size on both spans, so any box-height difference is the bug too.
