...
export async function loadBasicCases(): Promise<BenchmarkCase[]> {
  ...
  const cases: BenchmarkCase[] = [
    ...
    {
      // Issue #2: rounded corners on backgrounds AND border strokes. The
      // badge/tag cases only round backgrounds — a rounded border stroke
      // (uniform-border roundRect path) has no other pixel coverage.
